...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        ...
        with open(path,'r', encoding='utf-8') as file:
            ...
            for row in read_file:
                movies_data.append((int(row[0]),row[1],float(row[2])))

        connection.executemany("INSERT INTO movies VALUES (?,?,?)", movies_data)
        connection.commit()
    ...
    def part_aii_2(self,connection, path):
        ...
        with open(path,'r', encoding='utf-8') as file:
            ...
            movie_cast_data = []
            for row in read_file:
                movie_cast_data.append((int(row[0]),row[1],row[2],row[3],float(row[4])))

        connection.executemany("INSERT INTO movie_cast VALUES (?,?,?,?,?)", movie_cast_data)
        connection.commit()
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = '''SELECT printf('%.2f', COUNT(id) * 100.0 / 2484 ) AS PROPORTION 
                        FROM movies 
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = '''SELECT cast_name, COUNT(cast_name) AS appearance_count 
                        FROM movie_cast 
                        WHERE popularity > 10 
                        GROUP BY cast_name 
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = '''SELECT title, printf('%.2f', score), COUNT(cast_id) AS cast_count 
                        FROM movies INNER JOIN movie_cast ON id = movie_id 
                        GROUP BY id 
    ...
    def part_f(self,connection):
        ...
        part_f_sql = '''SELECT cast_id, cast_name, printf('%.2f', AVG(score)) AS average_score 
                        FROM movie_cast INNER JOIN movies ON movie_id = id 
                        WHERE score >= 25 
                        GROUP BY cast_id 
    ...
    def part_g(self,connection):
        ...
        part_g_sql = '''CREATE VIEW good_collaboration  AS 
                        ...
                        FROM movie_cast  MV1 INNER JOIN movie_cast MV2 ON MV1.movie_id = MV2.movie_id INNER JOIN movies ON MV1.movie_id = id 
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = '''SELECT Integrate_table.cast_id, MV.cast_name, printf('%.2f', Integrate_table.average_movie_score) AS best_collaborators
                          FROM ( SELECT gc1.cast_member_id1 AS cast_id, average_movie_score 
                                 FROM good_collaboration AS gc1 
                                 UNION ALL 
                                 SELECT gc2.cast_member_id2 AS cast_id, average_movie_score 
                                 FROM good_collaboration AS gc2 ) Integrate_table 
                          INNER JOIN movie_cast MV ON Integrate_table.cast_id = MV.cast_id
                          GROUP BY Integrate_table.cast_id
                          ORDER BY best_collaborators desc, MV.cast_name asc
                          LIMIT 5'''
        ######################################################################
    ...
    def part_h(self,connection,path):
        ...
        with open(path,'r', encoding='utf-8') as file:
            ...
            for row in read_file:
                ...
                connection.execute("INSERT INTO movie_overview VALUES (?,?)", data)
                connection.commit()
        ######################################################################
    ...
    def part_hi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hi_sql = '''SELECT COUNT(*) 
                         FROM movie_overview
                         WHERE overview MATCH "FIGHT OR Fight OR fight" AND overview != "gunfight" AND overview != "fighting" '''
        ######################################################################
